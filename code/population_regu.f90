...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:rm
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
      if ( P_establish(p)        <    0.000001        ) then; est_capacity(p)=.false. ;endif
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add (Temporal for experiment)
!   if (year == 1) then
!      x = 1.0
!   end if
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add (Temporal for experiment)
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
    ! When this is a plantation case, recruite occurs only once.
    if (plantation_flag == 1 .and. year > 1) then
       if (year < rec_ini_year) then
          x = 0.0
       end if
    end if
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   if (growth_calc) then
      dbh_sapwood (new) = 0.01*ALM5(pft(new))
      dbh_heartwood (new) = 0.01*(1-ALM5(pft(new)))
      height(new) = 2
      bole(new) = 0
      tree_h(new) = real(height(new))*STEP+1.3d0
      crown_diameter(new) = 1.0
      crown_area(new) = PI*(crown_diameter(new)/2)**2
      mass_leaf(new) = 1.0*crown_area(new)/SLA(pft(new))
!      mass_leaf(new) = optimum_dpai(pft(new))*height(new) * crown_area(new) / SLA(pft(new))
      mass_trunk(new) = (0.051d0*wood_rho(pft(new))*tree_h(new)                &
                        *(dbh_sapwood(new)*100.0d0                             &
                        +dbh_heartwood(new)*100.0d0)**2)*1000.0d0
      ! --- Use Komiyama
      mass_trunk(new) = (0.0696*wood_rho(pft(new))*tree_h(new)**0.931d0)       &
                        *1000.0d0
      ! ----------------
      mass_root(new) = mass_leaf(new)
      mass_coarse_root(new) = mass_root(new)*((1.0d0-fine_root_ratio(pft(new)))&
                              /fine_root_ratio(pft(new)))
      if (pr_s_a(pft(new)) < 0.0d0) then
         mass_above_root(new) = 1.0d0*mass_trunk(new)
      else
         mass_above_root(new) = 0.0d0
      end if
      mass_stock(new) = (mass_trunk(new)+mass_coarse_root(new))                &
                        *stock_trunk_ratio(pft(new))
      mass_stock_c(new) = mass_stock(new)*C_in_drymass/12.0d0
      mass_stock_n(new) = mass_stock_c(new)/c_n_leaf(pft(new))
      residual_c(new) = 0.0d0
      residual_n(new) = 0.0d0
   else
      dbh_sapwood (new) = 0.1*ALM5(pft(new))
      dbh_heartwood (new) = 0.1*(1-ALM5(pft(new)))
      height(new) = 37
      bole(new) = 7
      tree_h(new) = real(height(new))*STEP+1.3d0
      crown_diameter(new) = 5
      crown_area(new) = PI*(crown_diameter(new)/2)**2
      mass_leaf(new) = 3.0*crown_area(new)/SLA(pft(new))
      mass_trunk(new) = (0.051d0*wood_rho(pft(new))*tree_h(new)                &
                        *(dbh_sapwood(new)*100.0d0                             &
                        + dbh_heartwood(new)*100.0d0)**2)*1000.0d0
      mass_root(new) = 5000.0
      mass_coarse_root(new) = mass_root(new)*((1.0d0-fine_root_ratio(pft(new)))&
                              /fine_root_ratio(pft(new)))
      if (pr_s_a(pft(new)) < 0.0d0) then
         mass_above_root(new) = 0.05d0*mass_trunk(new)
      else
         mass_above_root(new) = 0.0d0
      end if
      mass_stock(new) = 100.0
   end if
   la(new) = mass_leaf(new)*SLA(pft(new))
   leaf_wp(new) = 0.0d0
   leaf_mdwp(new) = 0.0d0
   leaf_pdwp(new) = 0.0d0
   plant_water_uptake(new) = 0.0d0
   plant_sh_day(new) = 0.0d0
   plant_lh_day(new) = 0.0d0
   plant_gpp_day(new) = 0.0d0
   plant_npp_day(new) = 0.0d0
   plant_wnpp_day(new) = 0.0d0
   plant_fnpp_day(new) = 0.0d0
   npp_bottom_c(new) = 0.0d0
   npp_bottom_n(new) = 0.0d0
   mort_gpp(new) = 0.0d0
   crown_limit_flag(new) = .false.
   tree_n_est(pft(new)) = tree_n_est(pft(new))+1
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!_____________ Return when this site does not have satisfies minimum-fuel-load
   ...
   x = x * C_in_drymass / (Max_loc * Max_loc) !!!>>>>>>>>>>>>TN:rm <*********** add
!   x = x * C_in_drymass / real(GRID%Area) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!_____________ When fire occurs
   ...
   x = Min_gmass_stock * ((Max_loc/DivedG)**2) !x: minumum grass stock biomass (gDM/cell) !!!>>>>>>>>>>>>TN:rm <*********** add
!   x = Min_gmass_stock * (real(GRID%Area)/real(GRID%N_tot)) !x: minumum grass stock biomass (gDM/cell) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!Local variables (NEW)
   ...
   real,dimension(DivedG,DivedG)::grass_avandance !relative avandance of grass (fraction) !!!>>>>>>>>>>>>TN:rm <*********** add
   real,dimension(DivedG,DivedG)::fire_potential  !potential fire intensity (KJ s-1 m-1)  !!!>>>>>>>>>>>>TN:rm <*********** add
!   real,dimension(GRID%N_x,GRID%N_y)::grass_avandance !relative avandance of grass (fraction) !!!<<<<<<<<<<<<TN:add <*********** rm
!   real,dimension(GRID%N_x,GRID%N_y)::fire_potential  !potential fire intensity (KJ s-1 m-1)  !!!<<<<<<<<<<<<TN:add <*********** rm
...
!_____________ Calculate potential fire intensity
   !Unit converters
   Unit_conv1 = 1.0 / Max_loc  / Max_loc    !(stand-1 ---> m-2) !!!>>>>>>>>>>>>TN:rm <*********** add
   Unit_conv2 = 1.0 / ((Max_loc/DivedG)**2) !(grass_cell-1 ---> m-2) !!!>>>>>>>>>>>>TN:rm <*********** add
!   Unit_conv1 = 1.0 / GRID%Area    !(stand-1 ---> m-2) !!!<<<<<<<<<<<<TN:add <*********** rm
!   Unit_conv2 = 1.0 / (real(GRID%Area)/real(GRID%N_tot)) !(grass_cell-1 ---> m-2) !!!<<<<<<<<<<<<TN:add <*********** rm

   !Grass_avandance(Dived_G,Dived_G)
   !Relative grass avandance for each grass cell
   x = sum(gmass_leaf(:,:))/ DivedG/ DivedG  !!!>>>>>>>>>>>>TN:rm <*********** add
!   x = sum(gmass_leaf(:,:))/ real(GRID%N_tot)  !!!<<<<<<<<<<<<TN:add <*********** rm
...
!_____________ Determine whether fire occurs
   ...
   if ( sum(fire_potential(:,:)) / DivedG / DivedG < 300.0) return  !!!>>>>>>>>>>>>TN:rm <*********** add
!   if ( sum(fire_potential(:,:)) / real(GRID%N_tot) < 300.0) return  !!!<<<<<<<<<<<<TN:add <*********** rm
...
!   do j=1, GRID%N_y !!!<<<<<<<<<<<<TN:add <*********** rm
   ...
   if (tree_exist(no)) then
      ...
      i = int( DivedG * (bole_x(no) / Max_loc) ) !!!>>>>>>>>>>>>TN:rm <*********** add
      j = int( DivedG * (bole_y(no) / Max_loc) ) !!!>>>>>>>>>>>>TN:rm <*********** add
!      i = int( GRID%N_x * (bole_x(no) / GRID%Max_x) ) !!!<<<<<<<<<<<<TN:add <*********** rm
!      j = int( GRID%N_y * (bole_y(no) / GRID%Max_y) ) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   real, parameter :: vul_a = 4.0  ! Parameter for vulnerability curve for crowded crown
   real, parameter :: vul_b = 0.7  ! Parameter for vulnerability curve for crowded crown
   real, parameter :: vul_c = 4.0  ! Parameter for vulnerability curve for crowded crown
   real, dimension(Max_no) :: mort_crowded
   real(8) :: tree_h_min
   real :: mort_etc1
   real :: mort_etc2
   real :: mort_etc3
...
!_____________ Determine cohort structure
   ...
   End do
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ! Determine crowded trees based on actual and potential crown diamter ratio
   mort_crowded(:) = 0.0
   Do no=1, Max_no
   if ( .not. tree_exist(no) ) cycle
      ! Potential crown diameter (me, unit=m)
      d1 = crown_allometry ( &
                                           ! *** Input ***
      ...
      ! Motrality rate due to crowded condition
      mort_crowded(no) = M5(pft(no))*exp(-1.0*vul_a                            &
                         *((min(crown_diameter(no)/d1,1.0)+vul_b)/vul_b-1.0)   &
                         **vul_c)
   End do
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
   Do no=1, Max_no
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
         mort_greff = M1(p)/(1.0+M2(p)*x)
         ! Include the mortality rate due to crowded condition.
         mort_greff = max(mort_greff, mort_crowded(no))
         ! Add background mortality rate from M4.
         mort_greff = mort_greff + M4(p)
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   tree_h_min = min_height_allometry ( &
                                        ! *** Input ***
   ...
   ! For deformed trees due to stressful condition of salt
   if ( tree_h(no)>3.0d0 .and. tree_h(no)<tree_h_min ) mort_etc1=0.3
   ! For grown trees that cannot vertically develop crown
   if ( height(no)-bole(no)<=2 .and. age(no)>3   ) mort_etc2=0.1
   ! For vertically suppressed crowns
   if ( height(no)-bole(no)<=6 .and.                                           &
        dbh_heartwood(no)+dbh_sapwood(no)>0.1) mort_etc3=0.1
   ! For growth tree with very small crown area
   if (age(no) > 5 .and. crown_area(no) < 0.05d0) mort_etc4 = 1.0
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:rm
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:rm
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
         if ( mort_regu1(no)<0.0 .and. age(no)>3 ) mort_etc5=1.0
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
   End Select
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:rm
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   mort_total   = min(1.0, mort_greff+mort_lim+mort_etc+mort_etc1+mort_etc2+   &
                      mort_etc3+mort_etc4+mort_etc5)
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:rm (でいいかな？)
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:rm (でいいかな？)
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ! Kill tree if leaf or root biomass is almost zero
   if (mass_leaf(no)<1.1d0 .or. mass_root(no)<1.1d0 ) then
      death_list(no) = .true.
   end if
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   if (death_list(no)) then
      if (dbh_sapwood(no)+dbh_heartwood(no) > 0.00) then
         tree_n_die(p) = tree_n_die(p)+1
      end if
   end if
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
      mass_coarse_root(no) = 0.0
      mass_above_root(no) = 0.0
      mass_stock_c(no) = 0.0d0
      mass_stock_n(no) = 0.0d0
      residual_c(no) = 0.0d0
      residual_n(no) = 0.0d0
      leaf_wp(no) = 0.0d0
      leaf_mdwp(no) = 0.0d0
      leaf_pdwp(no) = 0.0d0
      plant_water_uptake(no) = 0.0d0
      plant_sh_day(no) = 0.0d0
      plant_lh_day(no) = 0.0d0
      plant_gpp_day(no) = 0.0d0
      plant_npp_day(no) = 0.0d0
