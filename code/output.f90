...
If ( counter==1 .or.  (Flag_spinup_read .and. counter==1+Spinup_year*Day_in_Year) ) then
   ...
   LAT,',',LON,',',ALT,',',Simulation_year,',',PFT_no,',',a1,',',a2,',',a3,',',Max_loc !!!>>>>>>>>>>>>TN:rm <*********** add
!   LAT,',',LON,',',ALT,',',Simulation_year,',',PFT_no,',',a1,',',a2,',',a3,',',GRID%N_x !!!<<<<<<<<<<<<TN:add y�����̏��񂪂Ȃ��B�v���� <*********** rm
...
!*** 5th line (Physical status of radiation)
   ...
   par_direct,',',par_diffuse,',',par*sum(par_grass_rel(:,:))/DivedG/DivedG,',',rad,',',radnet_soil,',',radnet_veg !!!>>>>>>>>>>>>TN:rm <*********** add
!   par_direct,',',par_diffuse,',',par*sum(par_grass_rel(:,:))/GRID%N_tot,',',rad,',',radnet_soil,',',radnet_veg !!!<<<<<<<<<<<<TN:add <*********** rm
...
!*** 7th~ lines (PFT specific variables)
   ...
   i = int( sum(par_grass_rel(:,:)) / DivedG / DivedG ) !!!>>>>>>>>>>>>TN:rm <*********** add
!   i = int( sum(par_grass_rel(:,:)) / GRID%N_tot ) !!!<<<<<<<<<<<<TN:add�I <*********** rm
   i = max(1,i)
   if (pft_exist(C3g_no)) then
...
!*** 1st line
   ...
   tree_density = real(i) * ( (100.0/Max_loc)**2 ) !!!>>>>>>>>>>>>TN:rm <*********** add
!   tree_density = real(i) * ( 10000.0/real(GRID%Area) ) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
...
!*** 2nd line (Carbon strage & flux)
   ...
   a1 = sum(flux_c_uptake_RR (1:i)) * C_in_drymass /Max_loc/Max_loc/ 100.0 !carbon uptake   (Mg C/month/ha) !!!>>>>>>>>>>>>TN:rm <*********** add
!   a1 = sum(flux_c_uptake_RR (1:i)) * C_in_drymass /real(GRID%Area)/ 100.0 !carbon uptake   (Mg C/month/ha) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
   a2 = ( sum(flux_c_mnt_RR(1:i)) + sum(flux_c_mnt_RR(1:i)) + sum(flux_c_htr_RR(1:i)) + sum(flux_c_fir_RR(1:i)) ) &
                                    * C_in_drymass /Max_loc/Max_loc/ 100.0 !carbon emission (Mg C/month/ha) !!!>>>>>>>>>>>>TN:rm <*********** add
!                                    * C_in_drymass /real(GRID%Area)/ 100.0 !carbon emission (Mg C/month/ha) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
...
!!!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<TN:add <*********** rm
   ...
   End if
   End do
   c1 = c1 * C_in_drymass / Max_loc / Max_loc / 100.0 !!!>>>>>>>>>>>>TN:rm <*********** add
!   c1 = c1 * C_in_drymass / real(GRID%Area) / 100.0 !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm

   !Grass carbon (Mg C/ha)
   c2 = sum(gmass_leaf(:,:)) + sum(gmass_root(:,:)) + sum(gmass_available(:,:)) + sum(gmass_stock(:,:))
   c2 = c2 * C_in_drymass / 100.0 / (Max_loc**2) !!!>>>>>>>>>>>>TN:rm <*********** add
!   c2 = c2 * C_in_drymass / 100.0 / real(GRID%Area) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
...
!*** 4th line ~ (for each PFT)
   ...
   x = C_in_drymass / Max_loc / Max_loc / 100.0 !!!>>>>>>>>>>>>TN:rm <*********** add
!   x = C_in_drymass / real(GRID%Area) / 100.0 !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
   c_leaf     (:) = c_leaf     (:) * x
   c_trunk    (:) = c_trunk    (:) * x
   c_root     (:) = c_root     (:) * x
   c_stock    (:) = c_stock    (:) * x
   ...
   x = C_in_drymass / 100.0 / (Max_loc**2) !!!>>>>>>>>>>>>TN:rm <*********** add
!   x = C_in_drymass / 100.0 / real(GRID%Area) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
   if (pft_exist(C3g_no)) then
   ...
   if (pft_exist(p)) then
      write (Fn,'( 7(f9.4,a) )') &
      sum(gpp_RunningRecord (1:i,p)) * C_in_drymass / Max_loc / Max_loc / 100.0,',', & !!!>>>>>>>>>>>>TN:rm <*********** add
      sum(npp_RunningRecord (1:i,p)) * C_in_drymass / Max_loc / Max_loc / 100.0,',', & !!!>>>>>>>>>>>>TN:rm <*********** add
!      sum(gpp_RunningRecord (1:i,p)) * C_in_drymass / real(GRID%Area) / 100.0,',', & !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
!      sum(npp_RunningRecord (1:i,p)) * C_in_drymass / real(GRID%Area) / 100.0,',', & !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
...
!Local variables
   ...
   real, parameter :: area_plot = (7*7*3.1415)/(30*30)      !!!>>>>>>>>> MY:add From 30m * 30m to 7m radius circle

!_____________ Unit converter: gDM/stand -> MgC/ha
   unit_conv = C_in_drymass * (100/Max_loc) * (100/Max_loc) / (1000*1000) !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   unit_conv = C_in_drymass / real(GRID%Area) / 100.0 !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N >>>>>>>>> MY:rm
...
!_____________ Prepare output variables
   ...
   ba          = ba /Max_loc/Max_loc                !BA (cm2 m-2) !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   ba          = ba / real(GRID%Area)                !BA (cm2 m-2) !!!<<<<<<<<<<<<TN:add >>>>>>>>> MY:rm
...
!_____________ Write title and data ***
   ...
   tree_counter * ( (100.0/Max_loc)**2 )     , ',', & ! 2 Tree density (ha-1) !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   tree_counter * ( 10000.0/real(GRID%Area) )     , ',', & ! 2 Tree density (ha-1) !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N >>>>>>>>> MY:rm
   ...
   write(*,*) '      year', '     tree number', '     height_mean', '     dbh_mean'   !  >>>>>>>>> MY:add
   write(*,*) year, tree_counter*area_plot, height_mean, dbh_mean                   !  >>>>>>>>> MY:add
...
SUBROUTINE output_forest_daily (Fn)
...
!Local variables
   real    height_mean    ! mean tree height (m)
   real    dbh_mean       ! mean dbh (m)
   real    agb            ! Above-ground biomass (kg)

   integer tree_counter   !tree counter
   integer no             !loop counter
...
!_____________ Prepare output variables
   ...
   write (Fn,'(i4,a, 4(f9.3,a))') counter, ',',real(tree_counter), ',',agb, ',',height_mean, ',',dbh_mean

END SUBROUTINE output_forest_daily
...
!Write data
   write (Fn,'(i4,a1)') count,',' !!!>>>>>>>>>>>>TN:rm <*********** add
!   write (Fn,'(i9,a1)') count,',' !!!<<<<<<<<<<<<TN:add <*********** rm
   Do i=1, Max_no
   if (tree_exist(i)) then
      write (Fn,'( 4(f6.1,a), 2(f7.3,a), 2(f7.4,a), i2,a )') & !!!>>>>>>>>>>>>TN:rm <*********** add
!      write (Fn,'( 4(f8.2,a), 2(f7.3,a), 2(f7.4,a), i2,a )') & !!!<<<<<<<<<<<<TN:add <*********** rm
...
!Write data
   ...
   Do i=1, Max_no
   if (tree_exist(i)) then
      write (Fn,'( i4,a, i2,a, i3,a, 9(f12.3,a))') &
      i                                   ,',', & ! Tree ID
      pft(i)                              ,',', & ! Species index (1: Rh, 2: Br)
      age(i)                              ,',', & ! Tree age (year)
      dbh_heartwood(i) + dbh_sapwood(i)   ,',', & ! DBH (m)
      tree_h(i)                           ,',', & ! Tree height (m)
      real(height(i) - bole(i) ) * STEP   ,',', & ! Crown depth (m)
      crown_diameter(i)                   ,',', & ! Crown diameter (m)
      mass_leaf(i)                        ,',', & ! Stand-level leaf biomass (g leaf/tree)
      mass_trunk(i)                       ,',', & ! Stand-level trunk biomass (g trunk/tree)
      mass_above_root(i)                  ,',', & ! Stand-level above-ground root biomass (g/tree)
      mass_coarse_root(i)                 ,',', & ! Stand-level coarse root biomass (g/tree)
      mass_root(i)                        ,','    ! Stand-level fine root biomass (g root/tree)
   end if
   End do
...
!Write data
   ...
   year, doy, sl_hgt(doy), dlen(doy), rad, par, par*sum(par_grass_rel(:,:))/DivedG/DivedG !!!>>>>>>>>>>>>TN:rm <*********** add
!   year, doy, sl_hgt(doy), dlen(doy), rad, par, par*sum(par_grass_rel(:,:))/real(GRID%N_tot) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!_____________ Unit converter2
   unit_conv = C_in_drymass * ( (100/real(Max_loc))**2 ) / (1000*1000) !gDM/stand -> MgC/ha!!!>>>>>>>>>>>>TN:rm <*********** add
!   unit_conv = C_in_drymass / real(GRID%Area) / 100.0 !gDM/stand -> MgC/ha !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
...
!_________________ Sumup leaf area for each tree
   ...
   End do
   ld1(:,:) = ld1(:,:) / Max_loc / Max_loc !adjust unit: (m2 step-1 stand-1) --> (m2 step-1 m-2) !!!>>>>>>>>>>>>TN:rm <*********** add
!   ld1(:,:) = ld1(:,:) / real(GRID%Area) !adjust unit: (m2 step-1 stand-1) --> (m2 step-1 m-2) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!adjust unit ( g dm / stand ---> Mg C / ha )
   biomass(:) = biomass(:) * C_in_drymass / Max_loc / Max_loc / 100.0 !!!>>>>>>>>>>>>TN:rm <*********** add
!   biomass(:) = biomass(:) * C_in_drymass / real(GRID%Area) / 100.0 !!!<<<<<<<<<<<<TN:add �P�ʗv�`�F�b�N <*********** rm
