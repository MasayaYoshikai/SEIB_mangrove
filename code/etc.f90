...
!_____________ Update of Ecosystem Total Carbon Pool running record
   ...
   pool_c_RR (1) = (x+y+z) * C_in_drymass / Max_loc / Max_loc /1000.0 !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   pool_c_RR (1) = (x+y+z) * C_in_drymass / real(GRID%Area) /1000.0 !!!<<<<<<<<<<<<TN:add >>>>>>>>> MY:rm
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
      sumup(p) = sumup(p) + mass_leaf(no) + mass_trunk(no) + mass_root(no) &
                 + mass_stock(no) + mass_coarse_root(no) + mass_above_root(no)
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
   end if
   End do
   ...
   sumup(:) = sumup(:) / Max_loc / Max_loc !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   sumup(:) = sumup(:) / real(GRID%Area) !!!<<<<<<<<<<<<TN:add >>>>>>>>> MY:rm
...
!_____________ Update LAI records
   ...
   do p = 1, PFT_no
      lai_each(p) = lai_each(p) / Max_loc / Max_loc !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!      lai_each(p) = lai_each(p) / real(GRID%Area) !!!<<<<<<<<<<<<TN:add >>>>>>>>> MY:rm
   end do
   ...
   lai_each(p) = sum(lai_grass(:,:)) / DivedG / DivedG   !!!>>>>>>>>>>>>TN:rm >>>>>>>>> MY:add
!   lai_each(p) = sum(lai_grass(:,:)) / real(GRID%N_tot)  !!!<<<<<<<<<<<<TN:add >>>>>>>>> MY:rm
