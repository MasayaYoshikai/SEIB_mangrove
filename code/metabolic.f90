...
!write(*,*) dbh_heartwood(no)+dbh_sapwood(no),mass_leaf(no),mass_trunk(no)
...
!DO j=1, GRID%N_y !!!<<<<<<<<<<<<TN:add <*********** rm
   ...
   x      = x * ((Max_loc/DivedG)**2) !gpp (g dm / cell / day) !!!>>>>>>>>>>>>TN:rm <*********** add
!   x      = x * (real(GRID%Area)/real(GRID%N_tot)) !gpp (g dm / cell / day) !!!<<<<<<<<<<<<TN:add <*********** rm
   ...
   canopy_cond = canopy_cond + &
                 ( GS_b1(p)*lai_grass(i,j) + const0*(2.0/eK(p))*const3 ) * ((Max_loc/DivedG)**2) !!!>>>>>>>>>>>>TN:rm <*********** add
!                 ( GS_b1(p)*lai_grass(i,j) + const0*(2.0/eK(p))*const3 ) * (real(GRID%Area)/real(GRID%N_tot)) !!!<<<<<<<<<<<<TN:add <*********** rm
...
canopy_cond = canopy_cond / Max_loc / Max_loc !!!>>>>>>>>>>>>TN:rm <*********** add
!canopy_cond = canopy_cond / real(GRID%Area) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!Local variables
   real,dimension(DivedG,DivedG):: lai_opt_grass   !optimum LAI for grass (m2/m2) !!!>>>>>>>>>>>>TN:rm <*********** add
!   real,dimension(GRID%N_x,GRID%N_y):: lai_opt_grass   !optimum LAI for grass (m2/m2) !!!<<<<<<<<<<<<TN:add <*********** rm
...
IF ( Phenology_type(p)==0 ) cycle
   ...
   select case ( Phenology_type(p))
     ...
     case (5:6) !grass
        flag(p)=.true.
        do k=1, 7
          y = sum(lai_opt_grass_RunningRecord(k,:,:)) / DivedG / DivedG !!!>>>>>>>>>>>>TN:rm <*********** add
!          y = sum(lai_opt_grass_RunningRecord(k,:,:)) / real(GRID%N_tot) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!      do j=1, GRID%N_y !!!<<<<<<<<<<<<TN:add <*********** rm
         ...
         lai_grass(i,j)       = gmass_leaf(i,j) * SLA(p) / ((Max_loc/DivedG)**2) !!!>>>>>>>>>>>>TN:rm <*********** add
!         lai_grass(i,j)       = gmass_leaf(i,j) * SLA(p) / (real(GRID%Area)/real(GRID%N_tot)) !!!<<<<<<<<<<<<TN:add <*********** rm

         !give minimum stock mass
         x = Stockmass_min_grass * ((Max_loc/DivedG)**2) !Minimum stock mass for grass layer (gDM/cell) !!!>>>>>>>>>>>>TN:rm <*********** add
!         x = Stockmass_min_grass * (real(GRID%Area)/real(GRID%N_tot)) !Minimum stock mass for grass layer (gDM/cell) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!_____________ Define local variables
   real   ,dimension((int(0.99999+Max_loc/20.0))**2, 5)::cohort_lai     !!!>>>>>>>>>>>>TN:rm <*********** add
   logical,dimension((int(0.99999+Max_loc/20.0))**2, 5)::cohort_crowded !!!>>>>>>>>>>>>TN:rm <*********** add
!   real   ,dimension(int(0.99999+GRID%Max_x/20.0)*int(0.99999+GRID%Max_y/20.0), 5)::cohort_lai      !!!<<<<<<<<<<<<TN:add �����ŗǂ����킩���Ȃ��B�v�m�F <*********** rm
!   logical,dimension(int(0.99999+GRID%Max_x/20.0)*int(0.99999+GRID%Max_y/20.0), 5)::cohort_crowded  !!!<<<<<<<<<<<<TN:add �����ŗǂ����킩���Ȃ��B�v�m�F <*********** rm
...
!$omp do private(no,x)
   ...
   If ( tree_exist(no) ) then
      ...
      id_location(no) = 1+int(bole_x(no)/20) + int(0.99999+Max_loc/20.0) * int(bole_y(no)/20)  !!!>>>>>>>>>>>>TN:rm <*********** add
!      id_location(no) = 1+int(bole_x(no)/20) + int(0.99999+GRID%Max_y/20.0) * int(bole_y(no)/20)  !!!<<<<<<<<<<<<TN:add �����ŗǂ����킩���Ȃ��B�v�m�F <*********** rm
...
!$omp do private(i,j)
   Do i=1, (int(0.99999+Max_loc/20.0))**2 !for each location !!!>>>>>>>>>>>>TN:rm <*********** add
!   Do i=1, int(0.99999+GRID%Max_x/20.0)*int(0.99999+GRID%Max_y/20.0) !for each location !!!<<<<<<<<<<<<TN:add �����ŗǂ����킩���Ȃ��B�v�m�F <*********** rm
...
!_____________ Initialize local variables
   ...
   Unit_conv = (Max_loc/DivedG)**2 !!!>>>>>>>>>>>>TN:rm <*********** add
!   Unit_conv = real(GRID%Area)/real(GRID%N_tot) !!!<<<<<<<<<<<<TN:add <*********** rm
...
!   integer,parameter::Crown_depth_min = 10 <*********** rm
   integer,parameter::Crown_depth_min = 5 ! <*********** add
...
!!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>TN:Add Mangrove feature ***tentative <*********** rm
...
!****************************************************************************************** add
  ...
  ! Set crown depth (m/tree) based on allometric relation obtained from field survey

  Case (7) ! Rh

     x = 9.5751 * (( dbh_heartwood(no)+dbh_sapwood(no) ) ** 0.7656)

     i = int( height(no) - x/STEP)

     count = max(0, i - bole(no))

  Case (8) ! Br

     x = 15.706 * (( dbh_heartwood(no)+dbh_sapwood(no) ) ** 0.6951)
...
!****************************************************************************************** add
  ...
  ! Set crown area (m2/tree) based on allometric relation obtained from field survey

  else if (Life_type(p) == 7) then ! Rh

     x = 8.218 * (diameter_new ** 0.6639)

  else if (Life_type(p) == 8) then ! Br

     x = 83.265 * (diameter_new ** 1.1848)
